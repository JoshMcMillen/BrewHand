...
/**
 * Create custom instructions for Copilot agent mode
 */
...
async function updateCustomInstructions() {
    if (!customInstructionsManager.hasCustomInstructions()) {
        const response = await vscode.window.showInformationMessage(
            'No custom instructions found. Would you like to create them?',
            'Create Instructions',
            'Cancel'
        );
        
        if (response === 'Create Instructions') {
...
async function removeCustomInstructions() {
    if (!customInstructionsManager.hasCustomInstructions()) {
        vscode.window.showWarningMessage('No custom instructions found in the current workspace.');
        return;
    }

    const response = await vscode.window.showWarningMessage(
        'Are you sure you want to remove BrewHand custom instructions? This will affect Copilot suggestions.',
        'Remove Instructions',
        'Cancel'
    );

    if (response === 'Remove Instructions') {
