    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('brewhand.formatCommand', async () => {
            ...
            if (input) {
                ...
                vscode.window.showInformationMessage(
                    `Formatted command copied: ${formatted}`
                );
            }
        }),
        vscode.commands.registerCommand('brewhand.toggleAutoMode', () => {
            ...
            if (isAutoBrewHandEnabled) {
                vscode.window.showInformationMessage(
                    '✅ BrewHand Auto Reminder enabled - Status bar will remind you to use @brewhand', 
                    'Open Chat', 'Got it'
                ).then(selection => {
...
async function checkLastTerminalCommand() {
    try {
        ...
        if (shellInfo.type === 'powershell') {
            ...
            vscode.window.showInformationMessage(
                '💡 BrewHand Tip: In PowerShell, use ";" instead of "&&" to chain commands',
...
function detectCommandsInPrompt(prompt: string): boolean {
...
function hasCompilationCommands(prompt: string): boolean {
...
function getShellAwareSystemPrompt(): string {
...
function getLanguageSupport(language: string): {
    ...
    const supportMatrix: Record<string, {
        tier: string;
        expectedAccuracy: string;
        limitations?: string;
        recommendedModel?: string;
    ...
    return supportMatrix[language as keyof typeof supportMatrix] || {
        tier: 'Tier 4 - Minimal',
...
- Follow Rust naming conventions and documentation`,
    ...
    return languageSpecific[language as keyof typeof languageSpecific] || `${commonStandards}
